...
public class TestPoolsCatalog : MonoBehaviour
{
    ...
    private Dictionary<Type, List<PoolItem>> _objectsFormPool = new Dictionary<Type, List<PoolItem>>();

    private void GetItem<T>(T obj) where T : PoolItem
    {
        var type = obj.GetType();
        _pool = _poolsCatalog.GetPool(obj);
        if (!_objectsFormPool.ContainsKey(type))
        {
            var listForDict = new List<PoolItem>();
    ...
    private void RelaseItem<T>(T obj) where T : PoolItem
    {
        var type = obj.GetType();
        _pool = _poolsCatalog.GetPool(obj);
        if (!_objectsFormPool.ContainsKey(type))
